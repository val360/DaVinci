...
Ext.define('GRIDEDITING.controller.Customers', {
    ...
    init: function () {
        this.control({
            'viewport': { render: this.onPanelRendered },
            'customerList': { edit: this.editCustomer },
            'checkcolumn': { checkchange: this.checkboxChanged },
            'customerList button[action=add]': { click: this.addCustomer }
        });
    },
    ...
    //called on customerList edit, calls validate then calls this.syncData to send updates via Ajax
